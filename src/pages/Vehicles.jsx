    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {alerts.map((a, i) => (
        <div key={i} style={{
          ...
          background: a.severity === 'high' ? THEME.accent.redDim : THEME.accent.yellowDim,
          border: `1px solid ${a.severity === 'high' ? THEME.accent.red + '44' : THEME.accent.yellow + '44'}`,
        }}>
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${hov ? 'rgba(240,120,20,0.3)' : THEME.border}`,
        borderRadius: 14, overflow: 'hidden', cursor: 'pointer', transition: 'all 0.18s ease',
      }}
    >
      <div style={{
        height: 100, background: `linear-gradient(135deg, ${vehicle.color} 0%, #0D0D0F 100%)`,
        ...
        <div style={{ position: 'absolute', bottom: 10, left: 14, fontSize: 10, color: 'rgba(255,255,255,0.5)', fontWeight: 600, letterSpacing: '0.15em', fontFamily: 'Rajdhani, sans-serif' }}>
          {vehicle.plate}
        </div>
      </div>
...
function MaintenanceRow({ entry, team, isLast }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 16, paddingBottom: isLast ? 0 : 16 }}>
      ...
      <div style={{ flex: 1, background: THEME.bg.card, border: `1px solid ${THEME.border}`, borderRadius: 10, padding: '12px 14px', marginBottom: isLast ? 0 : 0 }}>
