...
export function ProjectsModule() {
  ...
  const fresh = selectedProject ? (projects.find(p => p.id === selectedProject.id) ?? selectedProject) : null;

  // Mobile : détail plein écran
  if (isMobile && fresh) {
    return (
      <ProjectDetail
        project={fresh}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <style>{`@keyframes slideInPanel { from { transform: translateX(5%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }`}</style>
      ...
      {isMobile && (
        <div style={{ display: 'flex', padding: '8px 16px', borderBottom: `1px solid ${THEME.border}`, gap: 6, flexShrink: 0 }}>
          {WORKSPACES.map(ws => {
            ...
            return (
              <button key={ws.id} onClick={() => setWorkspace(ws.id)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 20, border: 'none', cursor: 'pointer', background: isActive ? `${ws.color}20` : 'rgba(255,255,255,0.04)', color: isActive ? ws.color : THEME.text.muted, fontSize: 12, fontWeight: isActive ? 700 : 500, fontFamily: 'inherit', transition: 'all 0.15s', outline: isActive ? `1px solid ${ws.color}44` : 'none' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', minHeight: 0 }}>
        {/* Board — panneau gauche */}
        <div style={{ flex: 1, overflow: 'hidden', padding: '20px 24px', minWidth: 0 }}>
          {view === 'kanban'
            ? <KanbanView
        ...
        {/* Panneau détail — glisse depuis la droite */}
        {fresh && (
          <div style={{
            width: '48%', maxWidth: 620, flexShrink: 0,
            borderLeft: `1px solid ${THEME.border}`,
            display: 'flex', flexDirection: 'column', overflow: 'hidden',
            animation: 'slideInPanel 0.22s cubic-bezier(0.25, 0.46, 0.45, 0.94)',
          }}>
            <ProjectDetail
              project={fresh}
              onBack={() => setSelectedProject(null)}
...
function KanbanView({ projects, team, onSelectProject, selectedProjectId, onAddProject, onMoveProject, onDeleteProject }) {
  ...
  const handleDrop = (col) => {
    if (draggingId !== null) {
      ...
      if (project && project.status !== col) onMoveProject(draggingId, col);
    }
    setDraggingId(null);
    setDragOverCol(null);
  };
    ...
    <div style={{ display: 'flex', gap: 16, height: '100%', overflow: 'auto', paddingBottom: 8 }}>
      {columns.map(col => {
        ...
        return (
          <div
            key={col}
            style={{ minWidth: 240, maxWidth: 300, flex: '0 0 240px', display: 'flex', flexDirection: 'column', gap: 8 }}
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flex: 1, overflowY: 'auto', borderRadius: 10, border: `2px dashed ${isOver ? THEME.accent.orange + '55' : 'transparent'}`, background: isOver ? 'rgba(240,120,20,0.04)' : 'transparent', padding: isOver ? 6 : 0, transition: 'all 0.15s', minHeight: 60 }}>
              {items.map(project => (
                <KanbanCard
                  key={project.id}
                  project={project}
  ...
  return (
    <div
      ...
      style={{
        ...
        border: `1px solid ${isSelected ? THEME.accent.orange + '77' : hov && !isDragging ? 'rgba(240,120,20,0.25)' : THEME.border}`,
        borderRadius: 10, padding: '14px',
        cursor: isDragging ? 'grabbing' : 'grab',
        transition: 'all 0.15s ease',
        borderLeft: `3px solid ${isSelected ? THEME.accent.orange : STATUS_CONFIG[project.status]?.dot ?? '#444'}`,
        opacity: isDragging ? 0.35 : 1,
        userSelect: 'none',
        boxShadow: isSelected ? `0 0 0 1px ${THEME.accent.orange}33, 0 4px 16px rgba(240,120,20,0.12)` : 'none',
      }}
    >
      {hov && !confirmDelete && (
        <button
      ...
      {confirmDelete && (
        <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', inset: 0, borderRadius: 10, background: 'rgba(0,0,0,0.88)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, zIndex: 2, backdropFilter: 'blur(2px)' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
            <button onClick={(e) => { e.stopPropagation(); onDelete(project.id); }} style={{ background: THEME.accent.red, border: 'none', borderRadius: 7, padding: '6px 16px', color: '#fff', fontSize: 12, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Supprimer</button>
            <button onClick={(e) => { e.stopPropagation(); setConfirmDelete(false); }} style={{ background: 'rgba(255,255,255,0.1)', border: `1px solid rgba(255,255,255,0.15)`, borderRadius: 7, padding: '6px 16px', color: '#fff', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Annuler</button>
          </div>
        </div>
      )}
...
function ListView({ projects, team, onSelectProject, onDeleteProject, selectedProjectId }) {
  return (
    <div style={{ overflow: 'auto', height: '100%' }}>
      <div style={{ display: 'flex', flexDirection: 'column', minWidth: 680 }}>
        ...
        {projects.map(project => <ListRow key={project.id} project={project} team={team} onClick={() => onSelectProject(project)} onDelete={onDeleteProject} isSelected={project.id === selectedProjectId} />)}
      </div>
    </div>
  );
}

function ListRow({ project, team, onClick, onDelete, isSelected }) {
  const [hov, setHov] = useState(false);
  const [confirmDelete, setConfirmDelete] = useState(false);
  const daysLeft = project.due_date ? Math.ceil((new Date(project.due_date) - new Date()) / 86400000) : null;
  return (
    <div
      ...
      onMouseEnter={() => setHov(true)}
      onMouseLeave={() => { setHov(false); setConfirmDelete(false); }}
      style={{ display: 'grid', gridTemplateColumns: '1fr 120px 120px 100px 140px 80px 32px', padding: '12px 16px', borderRadius: 8, cursor: 'pointer', background: isSelected ? `${THEME.accent.orange}0F` : hov ? THEME.bg.cardHover : 'transparent', border: `1px solid ${confirmDelete ? THEME.accent.red + '44' : isSelected ? THEME.accent.orange + '66' : hov ? 'rgba(240,120,20,0.15)' : 'transparent'}`, alignItems: 'center', transition: 'all 0.15s', marginBottom: 2 }}
    >
      <div>
      ...
      {confirmDelete ? (
        ...
        <button onClick={(e) => { e.stopPropagation(); setConfirmDelete(true); }} style={{ background: 'transparent', border: 'none', color: hov ? THEME.text.muted : 'transparent', cursor: 'pointer', fontSize: 18, lineHeight: 1, padding: '2px 4px', borderRadius: 4, transition: 'color 0.15s', justifySelf: 'center' }}
...
function NotionProp({ label, children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', minHeight: 30, padding: '3px 0', borderBottom: `1px solid rgba(255,255,255,0.04)` }}>
...
function InlineSelect({ value, options, renderValue, renderOption, onChange }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
      <div onClick={() => setOpen(o => !o)} style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 4, borderRadius: 5, padding: '2px 6px', transition: 'background 0.15s' }}
      ...
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, zIndex: 500, background: THEME.bg.card, border: `1px solid ${THEME.border}`, borderRadius: 8, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.55)', minWidth: 160 }}>
          {options.map(opt => (
            <div key={opt} onClick={() => { onChange(opt); setOpen(false); }} style={{ padding: '8px 14px', cursor: 'pointer', background: opt === value ? 'rgba(255,255,255,0.05)' : 'transparent', transition: 'background 0.1s' }}
...
function ProjectDetail({ project, onBack, team, currentMember, updateTaskStatus, deleteTask, addTask, addComment, updateProject, panelMode = false }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* ── Header ── */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 20px', borderBottom: `1px solid ${THEME.border}`, flexShrink: 0, background: 'rgba(255,255,255,0.01)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0, flex: 1 }}>
          {!panelMode && (
            <>
              <button onClick={onBack} style={{ background: 'none', border: 'none', color: THEME.accent.orange, cursor: 'pointer', fontSize: 12, fontWeight: 700, padding: 0, fontFamily: 'inherit', flexShrink: 0 }}>← Projets</button>
              <span style={{ color: THEME.text.muted, fontSize: 12, flexShrink: 0 }}>/</span>
            </>
          )}
          <span style={{ color: THEME.text.muted, fontSize: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{project.name}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
          <Btn size="sm" variant="secondary" onClick={() => setShowEdit(true)}>Modifier</Btn>
          {panelMode && (
            <button onClick={onBack} style={{ background: 'rgba(255,255,255,0.06)', border: `1px solid ${THEME.border}`, borderRadius: 7, width: 28, height: 28, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: THEME.text.muted, fontSize: 18, lineHeight: 1, flexShrink: 0 }}>×</button>
          )}
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '24px 24px 0' }}>
        ...
        {/* Titre */}
        <div style={{ fontSize: panelMode ? 24 : isMobile ? 26 : 34, fontWeight: 900, fontFamily: 'Rajdhani, sans-serif', color: THEME.text.primary, lineHeight: 1.15, marginBottom: 10 }}>
          {project.name}
        </div>
        ...
        {/* Propriétés */}
        <div style={{ marginBottom: 20, padding: '8px 0', borderTop: `1px solid rgba(255,255,255,0.05)`, borderBottom: `1px solid rgba(255,255,255,0.05)` }}>
          ...
          <NotionProp label="Priorité">
            <InlineSelect
              ...
              options={PRIORITIES}
              renderValue={(p) => { const cfg = PRIORITY_CONFIG[p]; return cfg ? <span style={{ fontSize: 11, fontWeight: 700, color: cfg.color }}>{cfg.label}</span> : <span style={{ fontSize: 11, color: THEME.text.muted }}>—</span>; }}
              renderOption={(p) => { const cfg = PRIORITY_CONFIG[p]; return cfg ? <span style={{ fontSize: 11, fontWeight: 700, color: cfg.color }}>{cfg.label}</span> : <span>{p}</span>; }}
        ...
        {/* Description */}
        {project.description && (
          <div style={{ marginBottom: 20, padding: '12px 16px', borderRadius: 10, background: 'rgba(255,255,255,0.025)', borderLeft: `3px solid rgba(240,120,20,0.35)` }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
          <div style={{ display: 'flex', gap: 2, background: 'rgba(255,255,255,0.05)', borderRadius: 8, padding: 3 }}>
            {[['tasks', `Tâches (${tasks.length})`], ['documents', `Docs${attachments.length > 0 ? ` (${attachments.length})` : ''}`]].map(([val, label]) => (
        ...
        {contentTab === 'tasks' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 24 }}>
            {tasks.length === 0 && <div style={{ textAlign: 'center', padding: '24px 0', color: THEME.text.muted, fontSize: 13 }}>Aucune tâche — cliquez sur + Tâche !</div>}
            {tasks.map(task => <TaskRow key={task.id} task={task} team={team} onToggle={updateTaskStatus} onDelete={deleteTask} />)}
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 24 }}>
            {attachments.length === 0 && !uploading && <div style={{ textAlign: 'center', padding: '24px 0', color: THEME.text.muted, fontSize: 13 }}>Aucun fichier joint.</div>}
        ...
        {/* Discussion */}
        <div style={{ borderTop: `1px solid ${THEME.border}`, paddingTop: 16, marginBottom: 0 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
            {comments.length === 0 && (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: THEME.text.muted, gap: 6, padding: '20px 0', opacity: 0.6 }}>
      ...
      {/* Input commentaire — épinglé en bas */}
      <div style={{ padding: '10px 20px', borderTop: `1px solid ${THEME.border}`, flexShrink: 0 }}>
        {currentMember
...
function AttachmentRow({ att, onDelete }) {
  ...
  const sizeStr = att.size ? att.size < 1024 * 1024 ? `${(att.size / 1024).toFixed(0)} Ko` : `${(att.size / 1024 / 1024).toFixed(1)} Mo` : '';
  const dateStr = att.created_at ? new Date(att.created_at).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' }) : '';

  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => { setHov(false); setConfirmDelete(false); }}
      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: THEME.bg.card, border: `1px solid ${hov ? 'rgba(240,120,20,0.2)' : THEME.border}`, borderRadius: 8, transition: 'all 0.15s' }}
    >
      <div style={{ width: 38, height: 38, borderRadius: 6, flexShrink: 0, background: 'rgba(255,255,255,0.04)', border: `1px solid ${THEME.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 800, color: THEME.text.muted, letterSpacing: '0.03em' }}>{ext}</div>
      ...
      <a href={att.url} target="_blank" rel="noreferrer" style={{ fontSize: 11, color: THEME.accent.orange, fontWeight: 600, textDecoration: 'none', flexShrink: 0, padding: '4px 8px', borderRadius: 5, background: THEME.accent.orangeDim }}>Ouvrir</a>
      {confirmDelete ? (
        ...
        <button onClick={() => setConfirmDelete(true)} style={{ background: 'transparent', border: 'none', color: hov ? THEME.text.muted : 'transparent', cursor: 'pointer', fontSize: 18, lineHeight: 1, padding: '2px 4px', borderRadius: 4, flexShrink: 0, transition: 'color 0.15s' }}
          onMouseEnter={e => e.currentTarget.style.color = THEME.accent.red}
          onMouseLeave={e => e.currentTarget.style.color = hov ? THEME.text.muted : 'transparent'}
        >×</button>
      )}
...
function TaskRow({ task, team, onToggle, onDelete }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => { setHov(false); setConfirmDelete(false); }}
      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 8, background: THEME.bg.card, border: `1px solid ${confirmDelete ? THEME.accent.red + '55' : THEME.border}`, opacity: done ? 0.6 : 1, transition: 'all 0.2s' }}
    >
      ...
      {confirmDelete ? (
        <div style={{ display: 'flex', gap: 5, alignItems: 'center', flexShrink: 0 }}>
          ...
          <button onClick={() => onDelete(task.id)} style={{ background: THEME.accent.red, border: 'none', borderRadius: 5, padding: '3px 9px', color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>Oui</button>
          <button onClick={() => setConfirmDelete(false)} style={{ background: 'rgba(255,255,255,0.08)', border: `1px solid ${THEME.border}`, borderRadius: 5, padding: '3px 9px', color: THEME.text.secondary, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>Non</button>
        </div>
      ) : (
        <button onClick={() => setConfirmDelete(true)} style={{ background: 'transparent', border: 'none', color: hov ? THEME.text.muted : 'transparent', cursor: 'pointer', fontSize: 18, lineHeight: 1, padding: '2px 4px', borderRadius: 4, flexShrink: 0, transition: 'color 0.15s' }}
          onMouseEnter={e => e.currentTarget.style.color = THEME.accent.red}
